...
function spinSlots() {
  if (canSpin == true) {
    ...
    if (totalBet == 0) {
      ...
      console.log("Insuffiecient funds");
    } else {
      playGame();
    }
  }
...
function findWinners() {
  ...
  for (let lineNum = 0; lineNum < numLines; lineNum++) {
    ...
    for (let j = numReels; j > 2; j--) {
      ...
      for (let i = 0; i < j; i++) {
        tilesInLine.push(
          parseInt(finalTiles[i][lines[lineNum][i]].split("e")[1])
        );
      }
...
function drawFinalReel(reelNum) {
  ...
  for (let j = 0; j < nTilesPerCol; j++) {
    ctx.drawImage(
      loadedImages[finalTileReel[j]],
      tileXPos,
      tileYPosArr[j] + slotFrames[j],
      tileSize,
      tileSize
    );
  }
  ...
  for (let i = 0; i < numReels; i++) {
    ...
    if (Math.floor(vPos[i]) < lastTilePos[i] + reelSpeed[i]) {
      ...
      for (let n = -2; n < endReelTile; n++) {
        ...
        if (
          yLoc > tileYPosArr[0] - tileSize &&
          yLoc < tileYPosArr[2] + tileSize
        ) {
          let rand = Math.floor(Math.random() * numPics);
          ctx.drawImage(
            loadedImages[Object.keys(loadedImages)[rand]],
            tileXPos,
            yLoc,
            tileSize,
            tileSize
