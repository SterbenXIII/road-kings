...
function displayBalance() {
  setDisplay("balance", balance);
}
function displayUpdatingBalance() {
  setDisplay("balanceUpdating", balUpdating);
}
function displayBet() {
  totalBet = numLines * betPerLine;
  setDisplay("betAmount", totalBet);
}
function displayPayout() {
  setDisplay("payout", payoutSum);
}
...
function incNumLines() {
  if (numLines < 50) {
    numLines += 1;
    drawAllLines(numLines);
  }
}
function decNumLines() {
  if (numLines > 1) {
    numLines -= 1;
    drawAllLines(numLines);
  }
...
function incNumAutoGames() {
  if ((numAutoGames + 1) * totalBet <= balance) {
    numAutoGames++;
  } else {
