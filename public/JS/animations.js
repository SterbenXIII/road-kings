...
function drawWinningLines(lineNum) {
  ...
  if (currentLine < numWins) {
    ...
    setTimeout(function () {
      lineWin.play();
      shrink = false;
      drawWinnerBackground(
        winningLines[currentLine],
        winningReels[currentLine]
      );
...
function drawLine(lineNum) {
  ...
  for (var i = 1; i < numReels + 1; i++) {
    var point = [
      xPos[i - 1] + slotWidths[i - 1] / 2,
      yPosArr[i - 1][lines[lineNum][i - 1]] + slotWidths[i - 1] / 2,
    ];
    pointsArr[i] = point;
  }
  pointsArr[numReels + 1] = [
    xPos[numReels - 1] + slotWidths[numReels - 1],
    yPosArr[numReels - 1][lines[lineNum][numReels - 1]] +
      slotWidths[numReels - 1] / 2,
  ];
...
function growAndShrink(lineNum, lastWinningReel = numReels) {
  ...
  for (let x = 0; x < lastWinningReel; x++) {
    let c = line[x];
    ctxAnimate.drawImage(
      loadedImages[finalTiles[x][c]],
      tileXPosArr[x] - (inc * t) / 2,
      yPosArr[x][c] + slotFrames[x] - (inc * t) / 2,
      tileSizeArr[x] + inc * t,
      tileSizeArr[x] + inc * t
    );
  }
