...
const ctxBtnTop = cDisplayHeight + cHeight,
  ctxBtnHeight = (cHeight * 2) / 5;
// canvasBtnBG.style.opacity = "0.8";
// canvasBG.style.opacity = "0.85"

if (
  canvasDisplayBG.getContext &&
  canvasBG.getContext &&
  canvasBtnBG.getContext
) {
  ...
  var BGImages = [];
  var backgroundPics = ["paluna.png", "roadKings.png", "reelBg.png", "controlBg.png"];
...
async function drawBG() {
  ...
  ctxDisBG.clearRect(0, 0, cWidth, cDisplayHeight);
  ctxBG.drawImage(BGImages["reelBg"], 0, 0, cWidth, cHeight);
  ctxDisBG.clearRect(0, 0, cWidth, ctxBtnHeight);
  // ctxBtnBG.drawImage(BGImages["controlBg.png"], 0, 0, cWidth, ctxBtnHeight);
  // for (let i = 0; i < xPos.length; i++) {
  //   ctxBG.drawImage(
  //     BGImages["backgroundSlotHD"],
  //     xPos[i],
  //     yPosArr[i][0],
  //     slotWidths[i],
...
function drawWinnerBackground(lineNum, lastWinningReel = numReels) {
  ...
  for (let x = 0; x < numReels; x++) {
    ...
    for (let y = 0; y < nTilesPerCol; y++) {
      if (y !== tileWinner) {
        ctx.drawImage(
          loadedImages[finalTileReel[y]],
          tileXPos,
          tileYPosArr[y] + slotFrames[y],
          tileSize,
          tileSize
        );
      }
